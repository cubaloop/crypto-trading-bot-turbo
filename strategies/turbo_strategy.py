...
class TurboStrategy:
    def __init__(
        ...
        bb_std: float = 2.0,
        atr_window: int = 7,
        signal_threshold: float = 0.28  # Umbral más sensible para scalping
    ):
    ...
    def compute_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 2. Fast ATR (7 períodos para scalping)
        high_low = df['high'] - df['low']
        high_close = (df['high'] - df['close'].shift(1)).abs()
        low_close = (df['low'] - df['close'].shift(1)).abs()
        tr = pd.concat([high_low, high_close, low_close], axis=1).max(axis=1)
        df['atr_7'] = tr.rolling(window=self.atr_window).mean()
    ...
    def generate_signal(
        ...
        if has_black_swan:
            return TradeSignal(
                ...
                stop_loss=snapshot.last_price * 1.01,
                take_profit=snapshot.last_price * 0.98,
                take_profit_2=snapshot.last_price * 0.96,
        ...
        if ohlcv_df is None or len(ohlcv_df) < self.bb_window:
            atr_approx = snapshot.last_price * 0.004
            return TradeSignal(
                symbol=snapshot.symbol,
                action="HOLD",
                conviction=0.0,
                entry_price=snapshot.last_price,
                stop_loss=snapshot.last_price - atr_approx,
                take_profit=snapshot.last_price + (1.2 * atr_approx),
                take_profit_2=snapshot.last_price + (2.5 * atr_approx),
                atr=atr_approx,
                reason="Inicializando buffer de velas rápidas"
            )
        ...
        atr = latest.get('atr_7', snapshot.last_price * 0.004)
        macd_fast = latest.get('macd_fast', 0.0)
        stoch_k = latest.get('stoch_k', 50.0)

        # 1. Señal de Ruptura de Squeeze (%B)
        # S_squeeze > 0 indica que el precio está rompiendo la banda superior
        ...
        # 4. Fusión Alfa Turbo
        # alpha_turbo = 0.35 * Squeeze + 0.35 * OBI_Accel + 0.20 * Momentum + 0.10 * NLP
        ...
        if conviction >= self.signal_threshold:
            action = "BUY"
            sl = snapshot.last_price - (0.8 * atr)  # SL ajustado para scalping
            tp1 = snapshot.last_price + (1.2 * atr)  # TP1 rápido
            tp2 = snapshot.last_price + (2.5 * atr)  # TP2 corredor
            ...
            action = "SELL"
            sl = snapshot.last_price + (0.8 * atr)
            tp1 = snapshot.last_price - (1.2 * atr)
            tp2 = snapshot.last_price - (2.5 * atr)
            ...
            tp1 = snapshot.last_price + (1.2 * atr)
            tp2 = snapshot.last_price + (2.5 * atr)
            reason = f"⏸️ Rango Neutral Turbo (Convicción: {conviction:+.2f})"
